...
occupancy_grid map;
location_t baseLocation;
location_t robotLocation;
...
void slam_backend::read_network(const std::string &read_json)
{
	try {
		...
		location_t newLocation = baseLocation + location_t(x, y, angle);
...
int main(int argc, char *argv[])
{
	...
	baseLocation = location_t(map.size()/2, map.size()/2, 0.0);
	robotLocation = location_t(map.size()/2, map.size()/2, 0.0);
